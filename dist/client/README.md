...
O workflow `update-ligamagic-prices.yml` roda a cada hora e, durante a primeira cobertura, consulta até 300 cartas por execução por no máximo 50 minutos. As requisições são sequenciais, com intervalo aleatório de 10 a 12 segundos e sem paralelismo. Cartas de duas faces são procuradas primeiro pelo nome da face principal; falhas determinísticas deixam a fila após duas tentativas. Dois bloqueios 403/429 consecutivos interrompem o lote e pausam a coleta por seis horas. Depois de completar o catálogo, o coletor passa automaticamente à manutenção de 100 cartas a cada 12 horas. Falhas preservam o último valor como desatualizado; cartas sem oferta ficam marcadas como indisponíveis. Os índices de preço e os detalhes particionados por `oracle_id` são atualizados diretamente no GitHub, sem republicar o site.
