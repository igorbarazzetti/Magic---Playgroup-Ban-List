...
function bind() {
  ...
  let searchTimer; $('searchInput').addEventListener('input', (event) => { clearTimeout(searchTimer); searchTimer = setTimeout(() => { state.query = event.target.value.trim(); syncUrl(); applyFilters(); }, 240); });
  ...
  $('openFilters').addEventListener('click', () => $('filterPanel').classList.add('is-open')); $('closeFilters').addEventListener('click', () => $('filterPanel').classList.remove('is-open'));
  ...
  document.addEventListener('keydown', (event) => { if (event.key === '/' && document.activeElement?.tagName !== 'INPUT') { event.preventDefault(); $('searchInput').focus(); } if (event.key === 'Escape' && $('filterPanel').classList.contains('is-open')) $('filterPanel').classList.remove('is-open'); if (event.key === 'Tab' && $('cardModal').open) { const focusable = [...$('cardModal').querySelectorAll('button,a')].filter((item) => !item.hidden); if (event.shiftKey && document.activeElement === focusable[0]) { event.preventDefault(); focusable.at(-1).focus(); } else if (!event.shiftKey && document.activeElement === focusable.at(-1)) { event.preventDefault(); focusable[0].focus(); } } });
